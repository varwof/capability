...
1. **Two independent implementations can be checked against one vector file.**
   Current state: 107 vectors (`syntax` (9) / `entail` (40) / `intersect` (14)
   / `decide` (44)),
...
|---|---|---|---|
| P1 Minimal core | this document §2 | 107 vectors; the previously orphan rule has a clause and a vector | ok |
...
| P5 Determinism incl. reason | §9.1 (pre-check + 11 layers), §9.2 (21 codes), §12.1 | 107 vectors assert the canonical reason (incl. §9.1 multi-grant aggregation, rev CLC-1.3) | ok — the 6 undocumented codes and the revision rule were added; the layer table now matches the numbering the corpus cites |
...
| P8 Separation | Appendix A | Go, Python and TypeScript agree on 107 vectors **and** 1184 property cases | **gap** — all three implementations share one author; third-party parity is the unproven half |
...
| P12 Agreement is the bar | §12, §12.1, vectors README | 107 vectors with `result_*` assertions + property cases, CI in three repositories | **gap** — the corpus cannot see the same-author limitation |
