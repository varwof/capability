...
> 权威副本在 `data/_vectors/clc-v1/`（`vectors.json` 114 条、`property-cases.json` 1184 条、
...
**107 条向量 + 1184 条性质用例**，三个共享同一作者的实现（Go / Python / TypeScript）全部通过。
...
参考套件检验：`vectors.json` —— **114 条**向量，映射见附录 B —— 与 `property-cases.json`
...
用 `kind` 字段把这些组**重新归并**：
`kind=entail (39)` 覆盖 B.2（6）+ B.3（29）再加 4 条 scheme 压力测试向量
...
| B.3 参数 | 数值上界、枚举成员、数组逐元素、对象递归、键闭包、表示与归一化 | 29 |
