...
**120 条向量 + 1184 条性质用例**，三个共享同一作者的实现（Go / Python / TypeScript）全部通过。
**实现符合性与「本文件声称某个一致性类」是两件事。** 一个实现满足 §12 为该类列出的义务，就是符合 CLC-A，
它可以自行声称这一符合，与还存在多少别的实现无关；§12 另外为**本文件**的声称设了一条成熟度门槛
—— 两个独立实现在裁决与原因码上一致。**这条门槛在本修订中尚未满足**：如 §12「实现的独立性（诚实的范围）」
所述，README 里点名的三个实现共享同一作者，它们的一致是**对文本的回归测试**，不是独立验证。
本修订仍声称 CLC-A 为基线授权一致性类——它的义务可实现、并由公开语料检验——同时把"独立实现"这条门槛
记录为未满足。证据侧一致性类 **CLC-E 不声称**：它的关系、值文法、参考实现与语料都在本修订里。
...
| CLC-1.6 | 2026-09-14 | §4.3, §6.2, §10, §12, 附录 B | **`jcs-sha256` 现在是真正的 RFC 8785 实现。** 规范化序列化不再使用 `json.Marshal` 的 HTML 转义（它把 `&`、`<`、`>` 写成 `\u0026`、`\u003c`、`\u003e`）：对象成员按 UTF-16 码元排序（§3.2.3），字符串按 §3.2.2.2 转义（只转义 `"`、`\` 与控制字符），数字按 ECMAScript `Number::toString`（§3.2.2.3），无多余空白；非法 UTF-8 或孤立代理项直接报错，不再替换成 U+FFFD。**字节因此改变：任何包含 `&`、`<`、`>` 的素材，其 `clc-action:` 标识符与 Decision Record 输入摘要都会改变 —— 旧摘要不是 JCS，不得与新摘要直接比较。** 当 UTF-16 序与 UTF-8 字节序不一致时，非 ASCII 对象键的排序也会改变。孤立代理项的拒绝在**原始参数文本**上执行 —— 解码器会先替换成 U+FFFD —— 并由 `params-031`/`params-032` 钉住。CLC-A 的判定不变，CLC-1.4/1.5 输入仍可读；证据语料增至 32 条（新增 RFC 8785 `&` action-id 向量与断言导出 §10 `Satisfaction` 报告的 requirement 向量）。  **2026-09-15 复核更正**：摘要不再声称 CLC-A 已满足「独立实现」门槛 —— 以 §12 的诚实范围为准（三实现共享同一作者）；三实现的**解码参数路径**对 malformed Unicode 返回同一稳定拒绝，Python 的解码尺寸检查改用 JCS 序列化，与 Go/TypeScript 对齐。 |
| CLC-1.7 | 2026-09-15 | §6.2、§12、摘要 | **实现对齐，不是语义变更。** 三实现的**解码参数路径**对 malformed Unicode 现在返回与 raw 路径相同的稳定拒绝（`invalid_params_number`）；Python 的解码尺寸检查改为度量 JCS 序列化而不是序列化器的重新编码 —— 这两处都是实现与 §6.2 不一致，不是语言缺口。摘要不再声称 CLC-A 已满足「独立实现」门槛：以 §12 的诚实范围为准（三实现共享同一作者）。良构输入无变化。 |
| CLC-1.8 | 2026-09-15 | §6.2、§12.1、附录 B | **实现对齐，不是语义变更。** 三实现的**原始参数校验**改为按数字的 **JCS 形式**度量 §6.2 第 4 步的尺寸，而不是收到的写法：`1e-6` 记作 `0.000001`（比 token 多四字节），`1.0` 记作 `1`（少两字节）—— 原始边界因此不再出现「解码边界拒而原始边界放行」或反向的情况（`params-033`–`params-036` 把上限两侧都钉住）。TypeScript 另把字面 astral 字符按 Unicode 标量值计数（不再按 UTF-16 码元），并拒绝字面控制字符与孤立代理项，与 Go、Python 一致（`params-037`/`params-038`，同一字符串的字面与转义两种写法；`params-039` 对应字面控制字符）。同一修订另写明输入边界义务：§6.2 第 7 条要求在收到的文本上、任何解码之前执行第 1–5 步，并规定只暴露已解码值入口的实现**不得**被描述为拒绝 malformed Unicode；§11 规定 §6.2 的拒绝不可转移到已解码值上。同一修订在边界对齐之外还随附六条语料钉：`entail-007/-008`（类位置通配不是结尾动作通配，§5.1/§9.3 第 3 层）、`decide-035`（多 grant 覆盖时的残差义务并集，§9.1/§8.4）与 `nested-001/-002/-003`（键闭包与存在性递归进入嵌套对象，§6.2/§9.1 第 7 层），语料由 114 增至 120 条。仅文本，无裁决变更。 |
...
7. **malformed Unicode 是"收到文本"的属性。** 孤立代理项转义、或不是合法 UTF-8 的字节，
   都没有 JCS 形式（§3.2.2.2）：第 1 步无法规范化，输入即被拒绝（`invalid_params_number`）。
   该检查定义在**收到的八位组**上，**必须**在任何解码步骤之前执行，因为通用 JSON 解码器不会保留
   这一区别。例如 Go 的 `encoding/json` 会把孤立代理项转义与非法字节**都**替换成 U+FFFD，于是
   `{"s":"\ud800"}`、`{"s":"\ufffd"}` 与一个字面非法字节到达决策函数时是**同一个值** ——
   `{"s":"\ufffd"}` —— 拒绝已经无法恢复。因此：

   - **收到**输入的当事方**必须**在收到的文本上执行第 1–5 步，而不是在已解码值的重新序列化上执行；
   - 只暴露**已解码值**入口的实现，**不得**被描述为"拒绝 malformed Unicode"：它的裁决定义在
     交给它的那个值之上，而那个值可能已经被修复过。取文本的那个入口才是规范入口；实现**应当**让
     两个入口的命名不易被调用方混淆；
   - 两方若必须对裁决一致，**必须**对收到的文本一致，或对它的摘要一致。
...
- **`allow_unresolved` 是授权结果，不是证据。** 它标记的是一个**未决的授权（或策略）条件**：
  ...
  读作"证据仍不足"
- **§6.2 的拒绝不可转移到已解码值上。** 输入边界检查定义在收到的文本上（§6.2）。因此，需要复现
...
参考套件检验：`vectors.json` —— **120 条**向量，映射见附录 B —— 与 `property-cases.json`
...
每个实现声明一个语言修订 `CLC-<major>.<minor>` —— 本文档声明 **`CLC-1.8`**。
...
用 `kind` 字段把这些组**重新归并**：
`kind=entail (47)` 覆盖 B.2（8）+ kind=entail 下的 35 条 params 向量再加 4 条 scheme 压力测试向量
（`clinical-001/-002`、`payments-001`、`data-002`）；`kind=decide (50)` 覆盖 B.5
（34：30 条 `decide-*`、2 条 `revision-*`、2 条 `undeclared-*` 第 7 层向量，包含 9 条
残差/值文法裁决向量、重新固定的 `decide-019/-020/-024`、新增的 `decide-028/-029/-030`，
以及本轮补行的 `decide-018`（通配形状）、`decide-031/-032/-033/-034`（max_rows 值域）
与 `decide-035`（多 grant 残差并集））
再加 7 条组合裁决向量、`payments-002` 与 `data-001`、4 条 params 边界裁决向量
（`params-028/-029/-034/-036`）及 3 条嵌套键闭包向量（`nested-001/-002/-003`）；
...
| B.2 蕴涵 | 字面匹配、结尾通配、跨命名空间、缺段、字面不符 | 8 |
| B.3 参数 | 数值上界、枚举成员、数组逐元素、对象递归、键闭包、表示与归一化 | 39 |
| B.4 交集 | 多来源收窄、空上界、来源顺序无关、参数无关的标识符比较 | 10 |
| B.5 裁决 | 完整裁决路径、原因顺序、残差义务、修订兼容 | 34 |
