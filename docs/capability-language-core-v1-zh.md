...
> 权威副本在 `data/_vectors/clc-v1/`（`vectors.json` 107 条、`property-cases.json` 1184 条、
...
参考套件检验：`vectors.json` —— **107 条**向量，映射见附录 B —— 与 `property-cases.json`
...
每个实现声明一个语言修订 `CLC-<major>.<minor>` —— 本文档声明 **`CLC-1.7`**。
