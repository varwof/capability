...
- `data/_vectors/clc-v1/param-bounds-meet-vectors.json` — **27 vectors** for the
  §6.6 `BoundMeet` intersection of `param_bounds` (rev CLC-1.14/1.15): numeric
  ...
  recursion, the cross-family refusal (`invalid_params_binding`, CLC-1.15), the
  empty and unrepresentable meets, the empty-Bound identity and the cross-site
  refusal, with `param-bounds-meet-vectors.schema.json`.
- `data/_vectors/clc-v1/param-bounds-equality-vectors.json` — **11 vectors** for
  the §6.5 JSON type-sensitive enum equality (rev CLC-1.15): `true ≠ 1`,
  `"1" ≠ 1` in both directions, at the top level and inside `nested`, element-
  wise on array requests, with numbers comparing after §6.2 canonicalization
  (`1 = 1.0`), with `param-bounds-equality-vectors.schema.json`.
- `data/_vectors/clc-v1/constraint-union-collation-vectors.json` — **2 vectors**
  pinning the §7.1 UTF-8 byte-order collation (rev CLC-1.15): the non-BMP case
  that separates UTF-8 byte order from UTF-16 code-unit order, and duplicate
  folding / chain-order independence, with
  `constraint-union-collation-vectors.schema.json`.
- `data/_vectors/clc-v1/param-bounds-meet-property-cases.json` — **500 cases**
  for the §6.6 meet invariant (rev CLC-1.15): every successful meet authorizes
  only what **every** source authorizes (generated by
  `scripts/gen-param-bounds-meet-property-cases.py`; CI asserts the generated
  file is byte-identical to the committed one).
